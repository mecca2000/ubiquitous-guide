...
本方案设计 Docker 沙箱集成方案，参考 deer-flow 项目的沙箱架构，实现安全的代码执行环境。**已根据实际服务器配置（2GB RAM, 2 核）优化资源分配。**
...
| 超时终止机制 | 4 小时 | 超时处理（60 秒） |
| 输出捕获 | 4 小时 | stdout/stderr |
| 安全加固 | 4 小时 | 安全配置 |

**说明：** Node.js 执行暂不实现（节省资源）
...
| 任务 | 预计时间 | 输出 |
|------|----------|------|
| 与 Agent 集成 | 4 小时 | 集成代码 |
| 端到端测试 | 4 小时 | 测试报告 |
| 资源限制测试 | 4 小时 | 内存/CPU 验证 |
| 安全测试 | 4 小时 | 安全报告 |
...
container:
  image: openclaw/sandbox:latest
  memory: 512MB       # 原 2GB，大幅压缩
  memory_swap: 512MB  # 禁止 swap
  cpus: 0.5           # 原 1.0，半核运行
  pids_limit: 50      # 原 100，减少进程数
  network: isolated
  timeout: 60s        # 原 300s，缩短超时
  volumes:
    - /home/admin/.openclaw/workspace:/workspace:ro
    - /tmp/sandbox-output:/output
```
...
### 六、风险与缓解（已根据 2GB 内存调整）

| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| 容器逃逸 | 低 | 极高 | 非 root、seccomp、只读文件系统 |
| **内存溢出** | **高** | **高** | **严格 512MB 限制、OOM 保护** |
| **CPU 竞争** | **高** | **中** | **0.5 核限制、任务队列** |
| 网络攻击 | 低 | 高 | 网络隔离、白名单 |
| 数据泄露 | 中 | 高 | 敏感目录不挂载、审计日志 |
| **任务失败** | **中** | **中** | **超时 60 秒、重试机制** |
